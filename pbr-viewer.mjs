...
function syncIorControls(preserveNumber=false){
 const displayedIor=String(Number(currentIor.toFixed(3)));
 $('ior').value=displayedIor;if(!preserveNumber)$('iorNumber').value=displayedIor;
 $('specular').value=String(iorToSpecular(currentIor));
 $('iorReading').textContent=currentIor.toFixed(3);fill($('ior'));
}
function syncPresetSelection(){
 $('selectedPreset').textContent=currentPreset?presetLabels[currentPreset]:'직접 조절';
...
function updateMaterial(custom=true,preserveIorNumber=false){
 ...
 // Keep the precise IOR independent of the two sliders' display step sizes.
 material.ior=currentIor;material.specularIntensity=$('iorToggle').checked?1:0;syncIorControls(preserveIorNumber);
...
function applyPreset(name){
 const preset=presets[name];if(!preset||!material)return;
 const category=Object.keys(presetCategories).find(key=>presetCategories[key].some(p=>p.id===name));
 if(category!==activePresetCategory)showPresetCategory(category);
 ...
 currentIor=specularToIor(preset.specular??0);
 currentPreset=name;$('specimenName').textContent=presetCategories[category].find(p=>p.id===name).name.toUpperCase();updateMaterial(false);status(presetLabels[name]+' · 조명과 거칠기를 바꾸며 비교해보세요.');
}
...
});$('baseColor').addEventListener('input',()=>material&&updateMaterial());
function updateIorFromInput(input,commit=false){
 if(!material)return;
 const value=input.valueAsNumber,min=+input.min,max=+input.max;
 if(!Number.isFinite(value)){if(commit)syncIorControls();return;}
 if(!commit&&(value<min||value>max))return;
 currentIor=Math.min(max,Math.max(min,value));updateMaterial(true,!commit&&input.id==='iorNumber');
}
for(const id of ['ior','iorNumber'])$(id).addEventListener('input',event=>updateIorFromInput(event.target));
$('iorNumber').addEventListener('change',event=>updateIorFromInput(event.target,true));
