...
function scope({ stage, controls, signal, announce }) {
  ...
  return { stage, controls, clean, active: () => !local.signal.aborted,
    ...
    say: message => { if (!local.signal.aborted && typeof announce === 'function') announce(message); }
  };
}
...
function mountHick(args) {
  ...
  function finish() { phase = 'done'; setLocked(false); $(stage, '[data-stimulus]').textContent = '완료'; $(stage, '[data-stimulus]').setAttribute('aria-label', '이 조건 완료'); const message = `8회 기록 완료. 신호 전 응답 ${anticipations}회. 다른 조건으로 바꿔 비교해 보세요.`; status(stage, message); ctx.say(message); }
  function next() {
    ...
    ctx.later(() => { $(stage, '[data-stimulus]').textContent = symbols[target]; $(stage, '[data-stimulus]').setAttribute('aria-label', names[target]); phase = 'active'; shownAt = performance.now(); ctx.say(`${names[target]}. 해당 숫자키 또는 버튼으로 응답하세요.`); }, 650 + (trial % 3) * 170);
  }
  function respond(choice, device) {
    ...
    if (trial >= 2) { records.push({ n, device, time: elapsed, wrong }); showHistory(); }
  ...
  ctx.on(start, 'click', () => { n = Number(count.value); method = input.value; trial = 0; anticipations = 0; const rng = random(seed); deck = [Math.floor(rng() * n), Math.floor(rng() * n), ...shuffled(Array.from({ length: 8 }, (_, i) => i % n), rng)]; renderChoices(); setLocked(true); revealStage(stage); board.focus({ preventScroll: true }); next(); });
...
function mountFitts(args) {
  ...
  ctx.on(target, 'click', e => { e.stopPropagation(); if (!active) return; const elapsed = performance.now() - time; const endDevice = device(e); unlock(); if (startDevice === 'keyboard' || endDevice === 'keyboard') { status(stage, '키보드 연습 완료 · 포인터 이동 기록에서 제외'); ctx.say('키보드 연습 완료'); } else if (startDevice !== endDevice) { status(stage, '입력 도구가 바뀌어 이번 기록은 제외했습니다.'); } else { records.push({ width, distance: Math.round(distance), device: endDevice, time: elapsed, misses }); showHistory(); const message = `${ms(elapsed)} · 빗나감 ${misses}회 · ${inputName(endDevice)}`; status(stage, message); ctx.say(message); } });
...
function mountJakob(args) {
  ...
  ctx.on(start, 'click', () => { training = !trained; condition = training ? 'learned' : layout.value; round = 0; order = shuffled(jakobItems.map(item => item.id), random(41024)); layout.disabled = true; start.disabled = true; renderMenu(); revealStage(stage); next(); });
