...
function mountReview({stage,controls,signal,announce,state:saved}){
 ...
 function render(){const h=heuristics[rule-1],focused=stage.contains(document.activeElement)?document.activeElement:null,savedName=focused?.getAttribute('name'),savedAction=focused?.dataset.action,savedFeedback=focused?.classList.contains('review-feedback'),savedHeading=focused?.tagName==='H3';let content='';
  if(rule===1)content=`<h3>카메라 대여</h3><p>미러리스 카메라 + 표준 렌즈</p><div class="review-product"><span class="review-camera" aria-hidden="true"><svg viewBox="0 0 64 48" fill="none"><path d="M16 13V8h14v5M13 13H9a4 4 0 0 0-4 4v23h54V17a4 4 0 0 0-4-4H13Z" stroke="currentColor" stroke-width="1.5" stroke-linejoin="round"/><circle cx="32" cy="27" r="10" stroke="currentColor" stroke-width="1.5"/><circle cx="32" cy="27" r="5" stroke="currentColor" opacity=".5"/><path d="M49 20h4" stroke="currentColor" stroke-width="2" stroke-linecap="round"/></svg></span><div><strong>CAMERA 01</strong><span>1일 대여 · 매장 수령</span></div></div>${button(state.busy&&improved?'요청 처리 중…':'예약 요청','submit',state.busy&&improved?'disabled':'')}<div class="review-feedback" role="status">${improved?esc(state.status):''}${state.busy&&improved?'<span class="review-progress" aria-hidden="true"></span>':''}</div>`;
  if(rule===2)content=`<h3>${improved?'예약 정보':'RQST_META'}</h3><dl class="review-definition"><div><dt>${improved?'대여일':'start_at'}</dt><dd>화요일 10:00</dd></div><div><dt>${improved?'반납일':'end_at'}</dt><dd>수요일 10:00</dd></div><div><dt>${improved?'수령 장소':'pickup_node'}</dt><dd>${improved?'대여 데스크':'EQ_PICKUP_01'}</dd></div></dl>${button(improved?'예약 신청':'RQST_INIT','confirm')}<div role="status" class="review-feedback">${esc(state.status)}</div>`;
 ...
 function action(event){const element=event.target.closest('[data-action]');if(!element)return;const a=element.dataset.action;
  ...
  if(a==='submit'){if(state.busy)return;state.busy=true;state.status='예약 요청을 처리하고 있어요.';render();timer=setTimeout(()=>{state.busy=false;state.status='예약을 접수했어요. 대여 데스크에서 수령해주세요.';render();},2000);return;}
