...
import {EFFECTS, DEFAULT_STATE, CONTACT_MODES, getEffectUniforms} from './weathering-model.mjs?v=68cf935e8045';
import {createWeatherMaterial} from './weathering-surface.mjs?v=2e9caa5ed4cd';
...
const shared={
  uRunoff:{value:0},uWet:{value:0},uRust:{value:0},uMoss:{value:0},
...
function observe(){
  ...
  if(has('wet')&&active.length===1){title='지금 표면에 남은 수분';description='조명 핸들을 움직여 젖은 표면의 어두운 색과 매끈한 반사를 살펴보세요. 현재 젖음은 녹·이끼·물자국을 새로 만들지 않아요.';}
  if(!active.length){title='깨끗한 표면부터 시작해요';description='효과 버튼을 하나씩 켜거나 여러 개를 함께 켜서 표면의 변화를 비교해보세요.';}
  if(state.view!=='surface'){
    ...
    description=state.view==='layers'?'각 효과가 닿는 곳을 범례의 고유 색으로 보여줘요. 겹친 곳은 색이 섞여 보여요. 레이어를 하나씩 골라 분포를 살펴보세요.':'검정은 영향 없음, 흰색은 강한 영향, 회색은 중간값이에요. 관찰할 레이어를 바꾸어도 켜 둔 효과의 조합은 유지돼요.';
    if(state.inspect!=='all'&&!has(state.inspect))description=state.inspect==='rust'&&state.material==='plastic'?'플라스틱에는 녹이 생기지 않아 이 레이어는 비어 있어요.':EFFECTS[state.inspect].label+' 효과가 꺼져 있거나 강도가 0이라 이 레이어는 비어 있어요. 효과 버튼을 켜고 강도를 조절해보세요.';
  }
...
function sync(){
  ...
  shared.uLayer.value=diagnostic?{all:7,dust:1,wear:2,wet:3,rust:4,moss:5,runoff:6}[state.inspect]:0;
  ...
  let history='클릭해 여러 효과를 함께 켜세요. 현재 젖음을 꺼도 녹·이끼·물자국은 남아요.';
  if(effectEnabled('runoff')&&!effectEnabled('dust'))history='물자국은 먼지와 함께 켜야 씻김과 이동 흔적이 보여요.';
  if(state.material==='plastic'&&state.effects.rust)history='플라스틱에는 녹이 생기지 않아요. 강철로 돌아가면 녹 선택이 복원돼요.';
